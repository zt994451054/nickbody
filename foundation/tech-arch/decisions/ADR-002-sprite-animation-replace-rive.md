...
2026-08-12 的 ADR-003 保留本决策的核心原则：桌面常驻宠物使用系统原生的预烘焙透明帧，不引入 Rive 或常驻 3D 运行时。ADR-003 取代以下实现细节：

- 固定 8×11 整张图集改为每动作独立 sprite strip + manifest。
- SwiftUI 视口切片改为 Core Animation 播放，避免逐帧视图重建。
- 固定 Row 状态映射改为稳定语义动作 ID。
- 桌面帧与大厅/跟练 3D clip 必须来自同一生产 Rig 和 DCC action。

历史 8×11/16×11 资源只作为迁移期兼容，不再是新动作的发布合同。
