...
宠物使用生产级 3D 网格、骨架和离线动作资产。视觉设计必须保留以下可读的身体语义；具体骨名、层级、权重和导出合同由当前版本 Rig 规范定义，不在视觉规范中重复维护：

1. **头部与冠部**：头部质心清晰，双叶、双耳等左右标记能准确表达侧倾和转头；冠部必须能独立表现轻微次级运动。
2. **肩、肘与手**：肩带、上臂、肘和手端点在正面及侧面均可辨识，可自然完成抬肩、上举、挥手和叉腰；手臂不能依靠穿过头部或躯干来完成动作。
3. **连续躯干**：胸、腹、背和髋部构成连续可变形体积。弯曲和扭转时允许自然压缩与展开，但不得出现裂缝、远端拉扯或纸片式折痕。
4. **髋、膝与脚**：双脚自然收拢站立，髋膝链能支撑走路、跑步、深蹲和抬腿；动作接触阶段脚底保持稳定。
5. **地面锚点**：角色视觉脚底和运行时地面锚点一致，所有大厅、跟练和桌面预烘焙动作使用相同 anchor，避免动作切换时上下跳动。

人形小草当前生产合同见 `versions/v1.0.0/engineering/pet-character-rig-v2.md`。角色动作必须先通过八方位原尺寸逐帧审核，再进入产品资源。
...
*   **强化跟练特效**：演示绕肩或伸展时，由 RealityKit 场景或原生 UI 叠加肩膀和手部的粒子流光轨迹，使用户看清运动路径与到位方向；特效不得写入或改变骨骼姿势。
*   **丰富的桌面互动**：
...
2.  **绝对无真实画面**：UI 中禁止放置任何用于“对齐画面”的真实视频窗口。摄像头的使用完全是黑盒，仅在后台计算骨骼位置，并映射为深色背景（`sys.color.text-primary`，即 #2F2A33）上的 3D 用户替身；资产不可用时才回退绿色骨架虚线。这向用户传达了绝对的安全信号。
