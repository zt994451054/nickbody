...
5. 🔄 手工蒙皮：无权重区域合同与四方位预览已通过 AI 预审，等待用户确认；确认后生成首版权重候选并按 Gate 1 逐关节修正，全部通过后再跑 Gate 2。
6. 制作组合姿势并通过 Gate 3。
7. 接入最小 `PetAnimationGraph`，先只播放 `idle-neutral` 和现有六动作迁移样例。
8. 依次制作挥手、张望、伸展；每个动作单独 Gate 4 和用户审核。
9. 最后生成独立桌面 strips，经批准后切换正式资源。

当前执行第 5 项。v002 静态拓扑和 32 关节无蒙皮骨位均已获得用户批准；无权重区域合同已将 `41,887` 个顶点互斥分配到基础区域，并保留 9 个可重叠固定过渡 mask，合同与清单分别绑定批准输入哈希和语义哈希。两轮边界修正后，基础区域与过渡区四方位图通过 AI 预审，当前等待用户确认区域合同。用户确认前不生成权重或 Armature 绑定；后续蒙皮候选仍须通过权重数量、归一化、语义左右配对、区域隔离和逐关节原尺寸审核。Gate 1 全部通过前不得执行 Gate 2，变形、动作和正式资源替换仍未获批准。
