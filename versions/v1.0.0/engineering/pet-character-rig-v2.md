...
> 当前状态：CHANGE-031 已完成 H3.1 两种减面对照，选定 basic 80,000 面，成功获得人形 v1.0 / Tripo 原生 41 骨及五种预设动作，另完成 22 姿势、44 张双视角图。处理链路已跑通，但肩髋/胸腹变形与远端背部拉扯仍未通过，未获得面部控制。本轮消费 115 credits，余额 190。生产验收仍阻塞；正式资源和历史证据保持不变，详见 §8.0.11。
...
当前阻塞点是自然变形与表情控制。CHANGE-031 已在 basic 80k 网格上完成原生人形绑定与五段预设动作，接口链路成功，肩髋/胸腹和远端背部变形仍不合格，详见 §8.0.11。后续优先针对骨骼位置与权重影响修整；生产网格/完整关节认证仍待完成。旧路线、正式 S0/32 关节合同和资源保持不变，能力试验不能直接改名为 v002 或正式绑定。
