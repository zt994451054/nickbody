...
> 当前状态：CHANGE-057 K4 严格零残差 QP 局部不可行，已用 1/1；精确定位到一条已在既有容差内却被强制归零的折角。容差编译已对齐，K5 初始问题已准备；86 项测试通过，见 §8.0.36。模型仍未修好，源与正式资源不变。
> 后续计划：按 [膝部修复计划 §26](./pet-knee-repair-plan.md#26-change-057-k4-实测反馈与折角容差对齐) 完成 K5 有界非线性修正驱动与单次登记，再执行原/实测预测及真实门禁。K5 未登记/求解；不自动 C4，D 未启动，没有模型整体无解证明。
...
当前阻塞点是自然变形与表情控制，CHANGE-033 腰部场仍为加工 GLB，历史失败保留。CHANGE-057 K4 零残差 QP 局部不可行，已定位并修正折角编译严于已有容差的问题；K5 初始 QP 已准备，驱动/登记待完成，见 §8.0.36 / 修复计划 §26。K3 的 169 条实际失败仍未解决，模型未修好。P2 2/2、S 2/2、C1/Q1/C2/C3/K1/K2/K3/K4 各 1/1，不自动 C4，D 未启动，源、资源及正式合同不变。
