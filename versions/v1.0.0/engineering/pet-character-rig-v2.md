...
4. 🔄 已建立 32 关节 export skeleton 和 Blender control rig，结构、IK/FK 传播及 16 张原尺寸骨位图已通过 AI 预审，等待用户批准骨位。
5. 手工蒙皮，按 Gate 1 逐关节修正；全部通过后再跑 Gate 2。
6. 制作组合姿势并通过 Gate 3。
7. 接入最小 `PetAnimationGraph`，先只播放 `idle-neutral` 和现有六动作迁移样例。
8. 依次制作挥手、张望、伸展；每个动作单独 Gate 4 和用户审核。
9. 最后生成独立桌面 strips，经批准后切换正式资源。

当前执行第 4 项。v002 将身体与头保留为连续闭合壳层，并把叶冠作为带 9 mm 内部重叠的独立次级变形组件；身体可见面四边面比例为 100%，叶冠为 99.9467%，两者开放边、非流形边和退化面均为 0，9 个关节区均检测到 5/5 层目标环带。32 关节 export skeleton 与 Blender control rig 无蒙皮母版已生成，结构合同、左右镜像、IK/FK 传播、正式资源保护和 16 张原尺寸骨位图通过 AI 预审；当前等待用户批准骨位。用户批准前不得开始手工蒙皮，蒙皮、变形、动作和正式资源替换仍未获批准。
