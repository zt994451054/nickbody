...
> 当前状态：CHANGE-093 已定位六对稀疏点、四对参考曲线及 192 个接口曲线对应，见 §8.0.71 和[源布局 §8](./pet-character-source-layout.md)。四个目标点及相容分区仍缺，密集映射未准入；零新模型，生产门禁保持。
> 后续依据：[完整重建决策与工单](./pet-character-production-rebuild-decision.md)。B1、纯权重路线和更早关闭路线保持冻结；不开放旧余额。原生产合同与正式资源不变，不宣称整体无解，也不承诺当前 AI 能独立完成尚未展示的建模能力。
...
### 8.0.32 CHANGE-053 前置检查与 K1 实际保护验证

新增 34 条、68 个精确穿越见证均未发现原支持/位移盒下的距离上界阻断，
最小放松上界 0.0081769，高于 5e-6；固定子特征检查无阻断。该结果只是必要条件。
共面/边界包含式窄阶段已补齐，三套坐标五姿势接触 ID 一致，恢复清单保持 1,534。
浮点宽阶段未宣称形式化认证，旧接触及原保护全部保留。

新 R/F 已接入完整清单、稳定 ID 状态映射、提前硬拒绝、共享次数/时间预算和非线性修正。
46 项新测试、49 项冻结回归通过；真实源准备 134.366 秒通过，无需更改旧维数或证据。
单次 K1 冻结同一 C3 首方向、半个校正 CCD 步长及两个正间隙约束，最多 3 修正 / 600 秒。
仅一次原生修正精确 Solved，独立原单位 KKT 通过；硬违反降至 2.9892e-11，
完整 CCD/面积/位移/接触检查通过，最大松弛改善 0.428653%。

同一权重在匹配操作顺序的 Blender 中，源对照、冻结区、静止及接缝坐标均逐位一致，
位移盒通过；但转身 66 条、跳跃 47 条新增区折角失败，最大超限
0.000180014 / 0.000392298 rad。113 条在仿射模型中均误判通过，候选拒绝。
终点复扫剩余 1,530 条原相交，转身 4 对分离且实测间隙约 1.05367e-5，无新增相交；
这不能抵消折角失败，也未完成原图或动画连续性验收。

K1 已用 1/1，不因内部只用一次修正而重启。下一项按修复计划 §22.5 验证具体失败顶点的
Blender 求值差异，再接入实际保护验收；不放宽 1e-6，不自动 C4。模型仍未修好，
没有整体无解证明。原源、正式资源及生产合同不变。

### 8.0.33 CHANGE-054 实际求值与候选验收

源/K1/C3 的五姿势均逐点复现。K1 的 113 条失败映射到 220 个 canonical 顶点；
去掉世界坐标往返舍入后，66/47 条转身/跳跃失败完全保留。
归一化位移并采用内部骨矩阵的双精度表达仍漏掉原失败中的 111 条，
不能作为 Blender 等价预测放行；详见修复计划 §23.2。

独立候选搜索已接入实际 Blender 新增区折角/位移门禁，先实际通过再判断收益和接受。
每次求值绑定具体权重、代码和二进制，独占输出并共享截止时间；
错配报告、输入变化、超时和失败都不能接受。折角 1e-6、严格物理位移盒不变。
72 项新旧测试通过，核心覆盖 94.77%；216.486 秒内的实际回归中，
源通过、K1 的 113 条和 C3 的 84 条失败全部拒绝，所有坐标与旧证据差为 0。

完整 R/F 未使用该新入口重跑；该门禁不替代实际接触复扫、动画连续性或原图验收。
K1/C3 各 1/1 不重启，没有新模型 QP、权重、C4 或正式资源替换。
下一项按修复计划 §23.5 完成 K2 固定段回退驱动与登记，
最多 4 候选 / 900 秒、零 QP；当前只有提案，尚未登记或执行。
模型仍未修好，无整体无解证明，生产合同保持。

### 8.0.34 CHANGE-055 固定段 K2 与单边非线性阻断

K2 按固定 1/2、1/4、1/8、1/16 四比例执行，在 61.339 秒内全部提前拒绝，
已用 1/1，无新 QP 或 Blender 求值。四步位移盒和两组保留间隙通过，
但唯一折角行 `protected-fold:2:57223:57352` 均超出 1e-6，
超限依次为 6.51111e-6 / 4.88716e-6 / 2.85186e-6 / 1.52805e-6 rad。
该行对应四个 canonical 顶点 25471、25507、25631、25795；
完整行审计排除辅助状态及其他约束类别，长双精度角度复核排除浮点误报。
源及 K1 数学端点通过，不能据此认为中间直线插值可行，详见修复计划 §24。

47 项新旧测试通过，核心覆盖 96.53%。原源与 K1 权重/清单逐位一致。
围绕最小冻结步已准备完整局部修正矩阵 105,372 × 13,578；
半径为独立步幅的 1/4，保留原阈值和中心收益上界，尚未求解。
下一项完成 K3 驱动与单次登记，最多 3 修正 / 600 秒，再过实际 Blender 和接触/收益检查。
K3 未登记/执行；K1/C3/K2 各 1/1 不重启，不自动 C4。模型仍未修好，
本轮未进入 CCD、Blender 或实际接触检查，不推断未测区间或模型整体无解。
原源、正式资源及生产合同不变。

### 8.0.35 CHANGE-056 K3 数学修正通过、实际保护拒绝

K3 已用 1/1：固定 K2 1/16 中心与完整约束，一次原生修正 / 一次 Blender，
264.705767 秒结束；29 新测试 + 26 门禁回归共 55 项通过。
冻结原单位矩阵、源状态和中心逐项一致；不放宽半径、收益上界、折角或物理位移盒。

数学硬违例降至 1.5901058248e-11，四姿势仿射 CCD、面积路径、静态接触和局部/保留间隙通过。
真实 Blender 仍有转身 88、跳跃 81 条保护折角超限；最大分别为
0.000174932637715 / 0.000155311927607 弧度。
源对照、冻结点、rest、接缝误差为 0，实际物理盒和支持/区外权重保持。
实际拒绝即停止，未进入实际接触/收益验收，不能称模型修好。

只读复核全部 169 行。原 K2 阻断面 57223/57352 数学低于上限 2.81760e-7，
实际却超限 9.18057e-5；应将实际残差纳入下一步修正，而非继续只追求数学可行。
下一项见修复计划 §25.4：先核对实测锚点预测及完整增广约束，再独立登记 K4；
当前仅提案，未准备/执行。预测误差尚无邻域保证，真实门禁仍必须执行。
K3 不重启、不自动 C4，D 和生产门禁状态不变，无整体不可行证明。

### 8.0.36 CHANGE-057 K4 局部不可行与既有折角容差对齐

K4 已用 1/1：完整实测锚点增广问题 150,940 行 / 13,578 变量；
一次 QP 在 41 内步返回 PrimalInfeasible，总计 72.879620 秒停止。
没有候选权重、路径检查或 Blender，本轮累计 86 项新旧/审计测试通过。

精确二进制有理数证书证明冻结的零残差线性问题在原盒内不可行。
其中跳跃面 70418/70988 初始残差 9.2631971649e-7，已低于已有 1e-6 门槛，
却被 QP 要求归零；其盒内最小线性残差仍为 4.1192972856e-7，单行已可阻断零目标。
这不证明既有门槛或真实模型不可行。K3 的 169 条实际失败仍未解决。

独立编译器已将原保护/实测两类折角上界对齐既有 1e-6：34,068 行调整，
其余 116,872 行、矩阵、目标及 K2 固定允许域不变。
K5 初始 QP 已准备，旧证书不再证明它不可行；完整可行性仍未验证。
下一项见修复计划 §26.4：先完成驱动与登记，再按既有门槛作有限非线性修正，
之后仍须全部原/实测预测、路径和真实保护/接触/收益通过。
K5 未登记/求解，K4 不重启，不自动 C4，D 与生产合同保持。

### 8.0.37 CHANGE-058 K5 局部线性不可行与 12 行折角冲突

K5 驱动、失败先行测试和独立登记完成；111 项新旧测试通过，新核心覆盖率 99.1416%。
完整 150,940 行 / 13,578 变量及源、K2 中心、K3 实测锚点精确复现。
两个折角族使用已有 1e-6 门槛，其余约束、严格物理盒与固定允许域不变。
最多 3 次严格改善修正 / 600 秒，至多一次 Blender；首个 QP 在 46 内步返回
PrimalInfeasible，监督 80.543966 秒停止，K5 已用 1/1，无候选权重或实际调用。

精确二进制有理数证书确认冻结线性问题在同一盒内不可行，分离量 1,006,408.6180969396。
169 个初始失败实测行均无单行盒阻断；仅原数学与实测折角两族仍可证明冲突。
进一步找到 12 行充分冲突子集：5 原数学、7 实测，转身/跳跃各 6 行，
涉及六组面片对，详见修复计划 §27.3；其精确分离量 1,388,451.765436459。
这不是全局最小冲突，也不证明非线性或真实 Blender 无解。

下一项按修复计划 §27.4 只读映射这组面片的源/K3 原数学与实际坐标、法线、导数、
支持及上限，分析坐标精度与非线性余项，形成有证据的修改后再提案。
当前未登记 K6、不追加 QP/Blender、不重启 K5、不自动 C4；D 与生产合同不变。
K3 的 169 条实际失败仍未解决，模型未修好。

### 8.0.38 CHANGE-059 局部预测误报与配对采样准备

12 行冲突映射到 8 顶点、9 三角面、24 个独立变量，原支持为 Spine01、
L_Clavicle、L_UpperarmTwist01/02，属于扩大问题的胸肩/左上臂保护区。
骨矩阵完全相同，几何导数最大相对差约 0.049954%；源至 K3 的坐标偏差变化
最大 4.50624e-8，被细薄面片放大成最大 1.75199e-4 弧度折角残差差异。
用已观察到的角点偏差作一阶解释，剩余最大 2.02844e-8；不构成未测邻域误差界。

这 24 个源权重变量位于 K5 投影盒内，12 个原数学/真实源折角均通过，
K3 锚点预测却误报 6 条实测行。完整源有 652 变量（含 35 权重）超出全域盒界，
因此该反例只否定局部预测与实际求值等价，不能宣称完整修复可行。
精确证书要求平均向下非线性余项至少 2.26310e-5 弧度；3 行暂无可用估计界，
不能把 K5 一阶证书升级为非线性或实际不可行结论。

16 个简单浮点/归一化对照均未复现实际残差。冻结代码存在网格→世界→骨架采集往返，
尚未证明是根因；已测试直接 float64 网格→骨架坐标函数，但未用于 Blender。
73 项测试通过，数学/采样核心覆盖率 95.9184% / 100%，本轮无新权重、QP 或 Blender。
下一项按修复计划 §28.4 补齐 K6 配对采集驱动与登记：最多 1 次 Blender / 600 秒，
仅两个冻结源/K3 输入场景；先复现旧路径，再比较同一几何的两条路径和全部原保护。
K6 未登记/执行，不替换既有门禁、不自动 C4，D 与生产合同保持，模型未修好。

### 8.0.39 CHANGE-060 K6 排除采集器替换方案

K6 一次 Blender / 50.5160715 秒完成两个冻结源/K3 场景，已用 1/1。
旧路径的坐标、骨矩阵、权重存储、动作样本、静态不变性及全部原保护精确复现。
直接路径等于求值网格局部坐标，完整失败集合仍为转身 88、跳跃 81；
两条路径的冻结/rest/接缝误差为 0，严格位移盒均通过。

12 行簇的源→K3 坐标偏差变化仍为 4.50624e-8，采集往返对运动偏差的贡献为 0；
折角残差往返贡献最多 4.23550e-12，剩余数学/实际偏差仍达 1.75199e-4 弧度。
已知源误报仍为 6 行。两条路径并非逐点相同，但更换采集路径没有消除当前失败。
不能据此认定具体 Blender 内部 bug，也没有修复不可行结论。

59 项测试通过，驱动/配对数学核心覆盖率 96.4286% / 100%。
独立 NumPy 2.5.3 复算有一个 2.22045e-16 的末位差异，精确校验拒绝；
保留失败后，使用随附 Python 3.11.9 / NumPy 1.24.3 只读重放精确通过，未重启 Blender 或改容差。
30 私有证据及 18 工具/汇总已绑定，App `064fd31` 已推送；无新权重、QP、导出或正式资产替换。

下一项见修复计划 §29.4：只读映射全量 169 行及边界，审计保护点保留原权重是否满足固定域和间隙条件，
先给出具体变量/约束表，再提出新修正登记；本轮按配对报告后停止的边界收束。
K7 未登记，不重启 K5/K6、不自动 C4，D 与原生产合同保持，模型未修好。

### 8.0.40 CHANGE-061 整体路线止损与交付里程碑

本轮按用户要求完成 [路线评审与执行总计划](./pet-knee-route-review.md)，
替代继续逐项追加 K 诊断的安排；历史证据和各次登记不变。
全量 169 行涉及 282 个规范顶点，其中 7 点的原权重在当前固定域外；
转身保留间隙还与部分点共享变量。不能直接恢复全部源权重，也没有完整可行性证明。

当前只有诊断推进，没有新增合格模型。后续纯权重路线共享 300 分钟主动工作、最多一次新试修：
R1 完整方案 ≤60 分钟，R2 实现/测试/真实局部验证 ≤120 分钟，
仅四姿态全部通过后开放 R3 连续动作和隔离交付 ≤120 分钟。
任一阶段失败或到时即停止当前路线自动投入，转入有证据和迁移成本的结构方案选择；
不再靠更换编号或追加工具诊断延长预算。

当前 native 41 骨试验仍不能直接晋升 S0、闭合身份或固定 32 关节生产资产；
R1 须说明继续试验的生产适配价值。原硬门槛、正式资源和全部认证顺序保持。
本轮无新候选、QP 或 Blender，R1 尚未开始，K7 未登记，模型未修好。

### 8.0.41 CHANGE-062 R1 拒绝与路线切换

R1 已完成源值保护点、目标接触、实际预测及生产适配核查，
完整决策见 [R1 方案结论](./pet-knee-r1-decision.md)，时间结算见路线评审 §6。
把全部失败保护点固定会保留 172 对目标穿插；关键八点在避免所查固定穿越时，
至少两点须可动、三条敏感折角涉及可动点。源基准预测又在已有 K3 数据上误放行六处实际失败。

本次单一自由度划分未形成能处理全部条件的完整实现方案，因此按原总计划拒绝 R1、关闭当前权重路线，
R2/R3 不启动，唯一试修保持 0/1；不把余量转为新 K 编号或结构试修额度。
下一项为有证据和生产适配成本的骨位/rest 与局部定向布线比较，不能将本轮拒绝写成必须改骨或重拓扑。
本轮无新权重、QP 或 Blender，模型仍未修好；S0、静态身份审核、固定 32 关节及全部正式门禁保持。

### 8.0.42 CHANGE-063 结构选择与完整施工包络

结构比较选择现有源表面的局部定向布线作为下一次有界施工方向；
没有把三角面数量或数值误差当成拓扑必然是根因，也没有再次凭单姿势相交数量移动骨位。
CHANGE-034 的虚拟中心后移已经试过，零相交仍有相邻折叠，不能作为未尝试方案重复投入。

只做膝部与肩臂小补片仅覆盖 254 / 1,534 对目标。新施工包络绑定 4,906 原面、
2,690 规范点、3,281 源点、七条简单边界，共 484 条边，Euler -5。
57 个右侧/掌指冻结点全部位于边界；原 2,189 可编辑点及肩臂八点全部在内部。
531 个 UV 接缝点需要保留源副本对应。七边界是施工选区，不是正式 S0 五接口；
连接、偶数边界和目标覆盖成立不代表四边布局或变形已经通过。

历史源超限面对中有三组完全处于原可编辑区外，仍记固定上下文，不计为修复通过。
完整 ID、量测、局限和生产迁移成本见 [结构方案](./pet-knee-structure-review.md)。

下一项 B1 最多一个候选、360 分钟：静态施工 120 分钟必须交付真实网格，
一套权重和实际四姿态 180 分钟，全部通过才开放连续性/隔离往返 60 分钟。
连同本次比较，结构路线总上限 420 分钟；阶段失败停止，不借旧余额或再开纯分析轮。
B1 未启动，本轮无新网格、权重、QP 或 Blender，模型没有改善。
native 41 骨仍是 vendor capability trial；S0、身份、32 关节及正式生产合同不变。

### 8.0.43 CHANGE-064 B1 静态候选失败，关闭本次施工

施工前将遗漏的完整关节核心及一圈邻点纳入，登记 5,824 原面、九边界/538 边，
掌指/右侧冻结点仍不在可编辑内部。原范围及全部历史文件保持，未增加候选或总预算。
一次固定顶点位置的三角换边、几何 MILP 配对及四边换边产生真实隔离 Blend，
局部 2,484 四边面、856 三角面。来源、UV/raw normals、全点坐标、边界及 74,176 外部原面通过保全检查。

独立实际审计失败：8 个折叠四边面、1 对新增静态自交、最大局部面比例 12.5971；
左膝/髋/肩/肘合格真实闭环均为 0，核心及一圈极点分别为 69/58/163/29。
双向表面实测距离 0.00183258/0.00202429 超过固定 0.001，原图见新增裆部角痕。
支撑路线没有完整合格声明，记未认证；不因少数面或配对目标改善而通过。

30 项工具/Blender 几何测试通过；三个核心构造模块覆盖率 85%，不含 IO 编排。
真实源候选 1/1、几何 MILP 一次；Blender 三次分别为元数据查询、合成测试、实际资产物化/审计/出图。
没有权重 QP、新权重、新动作或运行时资产替换。该 Blend 静态 rig/modifier/group/action 数量均为零，
仍只属于 vendor capability trial，不能成为 scoped_wip/formal normalizer 输入或 S0 通过凭证。

按原阶段止损关闭 B1，未用静态额度及后续 180/60 分钟不再开放；完整证据和时间结算见结构方案 §9–10。
此次固定点位施工没有造出完整关节面流；没有验证允许内部点重排的完整定向重拓扑，不能推广为无解。
下一项只做完整环/支撑/过渡和生产 32 关节适配的路径决策，不再改本候选或要求必然人工接手。

### 8.0.44 CHANGE-065 完整重建生产路径决策

[完整重建决策与 DCC 工单](./pet-character-production-rebuild-decision.md)已完成来源、布局要求、
内部点重排、五接口、身份/UV、32 关节迁移及后续交付的统一说明。
选择完整 DCC 直接制作为生产方向，沿用正式小草身份；拟议拓扑种子仍须符合 licensed 来源合同，
Tripo 41 骨只作试验和运动参考，不直接晋级。

当前无实际完整面连接/点位设计、已确认建模执行能力或可信工期，故新施工不启动。
建议以 DCC 建模协作补足能力，AI 承担合同、独立验收和生产接入；没有联系他人或付费。
本文不授予新路线，不重开 licensed R0、B1 或 K 系列，不变更源审批、合同、阈值或正式资源。
原 licensed Blend 临时原件已不可用，本次官方 URL 恢复返回 HTTP 403，未来还需恢复同 hash 原件；
现有 intake 文字与哈希绑定不等于实际文件已经恢复。

本轮仅完成决策和文档，无新网格/权重/动作，没有新增模型通过；不证明模型无解或只能人工完成。
旧 manual-dcc-v001 的闭合流程及坐标说明不能覆盖现行合同，闭合身份专用 assessor 仍须在 Rig Gate 0 前补齐。

### 8.0.45 CHANGE-066 真实肩腋参考与迁移设计

用户无法寻求人类建模协作，已授权 AI 继续研究。独立合成肩部的 40° 基例、35° 及宽度 0.25
修复结果已有静态联合质量证据；这些不是本角色完整语义/S0，也不把原直接迁移失败改写成成功。
[真实肩腋迁移设计](./pet-character-shoulder-migration-study.md)绑定可用身份 GLB、五接口和正式合同，
记录 8,545 个只读参考面、564 个邻域保护面，及 s=0.16–0.20 m 的五个实际手臂截面。
四条拟用关节导向各 32 点，截面长短轴比约 1.45；曲线不是已构造的结构环。

下一项为最多 90 分钟、一份首版加一次同域修正的独立肩腋制作，详细范围/停止点见该设计 §4。
先通过源曲面参数域与完整面连接检查，再保存实际四边补片并独立审计；不恢复旧固定点位换边、
旧 builder 或分支管整体贴回。当前尚无真实新补片或正式来源变更，完整身体/S0/身份/骨架仍待完成。
原始参考未做 Head=0.90 和生产 A-pose，旧骨仅作切片方向线索；不将原始截面角度或接口最近距离当生产通过。
真实肩部 WIP 不可晋级；如将来使用独立直接构造作为正式来源，须先明确更新来源决策与合同，不能伪填 licensed 来源。

### 8.0.46 CHANGE-067 真实肩腋首版与布局拒绝

[迁移设计 §6](./pet-character-shoulder-migration-study.md)记录实际源图、首版与独立 Blender 审计。
源域 8,865 三角形 UV 翻折为 0，首版 480 点 / 432 四边面，单连通，两边界 64/32，保存重开一致，无自交。
但双对角线折面为 3/11（13 个不同面），最大边长比 24.40685；补片/源双向距离 P95 为 2.16664/4.45239 mm、最大为 5.85125/7.51667 mm。
四个 32 点实际导向环及连续环带成立，最小纵向对齐 0.93156，但不认证肩环解剖位置。
肩顶/腋下路径未连成连续成对四边带，32 个内部过渡极点侵入核心及一圈；原尺寸观感也拒绝。

本次按前 30 分钟完整布局未成立的条件结项：首版 1、修正版 0、非线性优化 0，实际静态审计 1。
另有 3 个只渲染同一保存网格的进程用于渲染环境修复和光照复核，无网格变化。
15 项有效工具测试通过，正式资源/接口/合同与 448 份旧私有文件、6 份既有源码绑定不变。
下一项先证明连续支撑带、胸背过渡极点和端口缓冲的离散连接设计；当前图不再进行位置试修。
本次 WIP 不晋级正式来源、S0 或 rig gate，也不从失败推导整体无解或必须人工接手。

### 8.0.47 CHANGE-068 支撑连接成立，几何拒绝

[迁移设计 §7](./pet-character-shoulder-migration-study.md)为当前接续入口，App 证据提交 `2c35d9d`。
1,024 点 / 976 四边面、单连通、冻结 64/32 两边界；实际两组支撑带连续，核心及真实一圈极点为零。
四个原导向的环几何通过，但仍未认证肩关节解剖位置。

双对角线折面 81/174（182 个不同面）、自交 37 对，均比上一版明显退步；最大边长比 16.15762。
补片→源 P95/最大为 1.59718/6.21099 mm，反向 3.61253/8.31830 mm；贴合与原尺寸观感未通过。
参数中点缓冲不等于计划的精确 `.195` 轴向站位，实际偏差及只读 UV 诊断限制见迁移设计。
本次只有连接结构取得进展，真实模型仍拒绝，不晋级 S0 或正式身份，不进行权重和动作。

预算内一设计/一实物/一静态审计、零修正/优化；19 项工具测试通过。16 项输入、501 份旧私有文件、
6 份既有源码绑定及正式资源保持。下一项验证非共线密度转换、周期接缝、真实缓冲和曲面行距的布点方法；
本轮不追加网格，不保证小调整能成功，完整角色生产仍阻塞。

### 8.0.48 CHANGE-069 固定图布点与几何退步

[迁移设计 §8](./pet-character-shoulder-migration-study.md)为当前入口，App 提交 `4f7d628`。
原 1,024 点 / 976 四边面、端口与四导向保持；精确 `.195 m` 缓冲站位落实，实际连续支撑带仍通过。
但核心及真实一圈有 10 个极点、自交 143 对，双对角线折面 61/73（91 个不同面）。
补片→源 P95/最大 `0.83697/14.58935 mm`，反向 `9.48255/18.92383 mm`，贴合及原尺寸观感拒绝。
不同折面减少不足以抵消自交、净空和覆盖误差退步，没有整体模型进展。

一次线性初值、一次 200 迭代/10.18105 秒求解、一份最终实物/一次静态审计，零后续修正版；26 项工具测试通过。
初值不满足核心净空，加权目标没有建立硬约束同时可行的起点；域限制和迭代结束也不是不可行证明。
下一项先在源曲面明确保护/转换分区及不交叉边界，再联合设计连接与布点，不追加本次加权试跑。
原 23 项输入、529 份旧私有文件、6 份 App 绑定及正式资源保持，S0/身份/32 关节与动画仍阻塞。

### 8.0.49 CHANGE-071 局部模板与源分区预检

[方法研究 §5](./pet-character-quad-method-research.md)及[迁移设计 §9](./pet-character-shoulder-migration-study.md)为当前入口。
独立模板生成六区域连接，1,264 点/1,216 四边面，256 原固定点保持；图上支撑带及原核心真实一圈净空通过，保护区极点为 0。
真实源坐标仍有 18 个对角线法向冲突面、最大边长比 45.79230；源三角区域覆盖与不交叉边界尚未认证，前置拒绝。
未导出新 Blend 候选或执行独立审计，不能把预检与 CHANGE-069 的实物指标作直接改善比较。

一套布局、两次预检（包含一次缓冲环起点读取修正，原失败保留），零形状优化/Blender，35 项测试通过。
下一项先完成按真实曲面度量划区与有效局部映射，本次不追加布局试跑。正式资源、合同、S0/身份/32 骨及动画状态保持未完成。

### 8.0.50 CHANGE-072 源度量分区与独立实物验收

当前接续见[方法研究 §6](./pet-character-quad-method-research.md)及[迁移设计 §10](./pet-character-shoulder-migration-study.md)。
原 8,865 源三角的辅助凸图及八区切片覆盖通过，256 个原固定点精确保持；唯一 1,264 点 / 1,216 四边面已独立保存重开。
原 75 mm 核心及真实一圈极点为 0，四环和成对实际支撑带通过；这些环仍为拟用上臂导向，未批准为生产肩关节位置。
相对 CHANGE-069，自交 143→13、不同折面 91→14，补片→源 P95/最大 0.49660/1.57245 mm 通过；
反向最大 4.17172 mm 仍失败，最大边长比由 12.42234 退步到 28.63180。几何、双向贴合及八张原图整体拒绝，不能晋升 S0 或骨架制作。

50 项测试通过，一次 Blender 审计，零优化/修版。下一项先验证固定近端拐角覆盖能力，再按实际宽度设计窄分区边数及腋下走向；本轮不再构造。
213 份保护文件及正式源/合同/资源保持，完整身体与身份/UV/32 骨及动画未完成。尚无固定接口无解或必须人工接手的结论。

### 8.0.51 CHANGE-073 固定近端边界覆盖诊断

[方法研究 §7](./pet-character-quad-method-research.md)和[迁移设计 §11](./pet-character-shoulder-migration-study.md)为当前接续。
保持原源图和 256 点，全部 35,460 个原审计参考采样可在 64 条近端参数边的允许内侧找到 ≤0.88005 mm 的源点，逐点重心坐标和归属已核验。
这是采样源点可达性，不能替代完整四边网格的贴合/形状/自交检查；固定接口不可行没有被证明。
一份移动四点的边界备选使连续曲线双向上界降至 1.25305/1.23292 mm，但未应用，原接口仍冻结。

下一项优先保留原固定点，设计拐角内部导向及按物理宽度分段的窄区连接，再作完整预检。本轮零新模型/Blender，59 项测试通过，265 项保护文件不变。
CHANGE-072 仍是最近失败实物；完整身体、S0、身份和 32 骨/动画未完成，本轮不晋升任何制作阶段。

### 8.0.52 CHANGE-074 实物退步与模板方法关闭

唯一 764 点 / 716 四边面保持原 256 点；九区覆盖与连接图通过。独立审计自交 13→3、最大边长比 28.63180→10.49574，但不同折面 14→15，原核心真实一圈极点 0→1；补片→源最大误差 1.57245→3.21348 mm，反向 P95 1.69437 mm、最大 3.96267 mm、超限 1,385 点。四环及实际双支撑带局部通过，几何/贴合/保护区及八张原图整体拒绝。

执行时未遵守此前“物理预检失败即停止”规则，已如实记录；现补导出前物理拦截并对同一存储网格只读核验，零模型修正/审计重跑。73 项工具测试通过，298 项保护文件不变；App `41843ac`，详见方法研究 §8。
...
完整身体/S0/身份/32 骨/动画仍未完成，旧路线和正式资源保持。

### 8.0.53 CHANGE-075 外部方法能力检查

三类方法的证据及缺口见[方法研究 §9](./pet-character-quad-method-research.md)：Gmsh 文档具有仅补空实体的选项，但完整流水线的固定点/保护区保持未实测；Foldover-free maps 的已读入口为平面三角与体四面体；Mesh-Untangling 的受支持 CUDA 入口不适用当前 Mac，其他硬约束接口亦未核实。没有方法准入真实 Sprout 构造。

下一项仅为一次 Gmsh 独立小样、一次生成的固定实体能力检查，60 分钟总预算、20 分钟入口截止，尚未执行。坐标、面连接、保护区或几何检查任一失败即淘汰，不调参重跑；即使通过，也只进入真实源适配设计，不能晋升 S0、身份、关节或动画门禁。

本轮无新模型、示例执行、编译、Blender、优化或付费，347 项保护文件及正式资源保持，App 仍为 `41843ac`。研究阶段超时 347 秒已披露；实际文档推送及总工时见私有 `sprout-constrained-method-review-20260920/receipt.json`。原 256 点、75 mm 核心及真实一圈、四条拟用上臂导向和全部生产合同不变。

### 8.0.54 CHANGE-076 Gmsh 固定实体检查失败

Gmsh 4.15.2 Algorithm 11＋MeshOnlyEmpty 的单次圆柱小样只保留 10/104 个原固定坐标；原 64 个面和 56 段连接均未保留。原曲面重划为 252 面，接口从 8 段变为 14/18 段。比较器允许无损重编号，固定条件失败后按规则停止完整几何/贴合审计，入口关闭。

12 项纯检查器测试通过，零真实角色网格、Blender、付费或骨架。372 项保护文件与原生产合同/资源保持，App `6004e40`；方法、环境、实际多阶段内部操作、证据和工时见[方法研究 §10](./pet-character-quad-method-research.md)。本轮不通过任何生产门禁。

下一项只核查固定四边块与空缺域分离的构造设计，30 分钟设计截止、60 分钟总预算，零新网格或工具试跑。新方向尚未证明可行；本次 Gmsh 接入及 CHANGE-074 方法不重开。

### 8.0.55 CHANGE-077 固定核心与配对设计未准入
...
### 8.0.58 CHANGE-080 域分离与公共接口核查

按[核心约束设计 §9](./pet-character-core-constraint-design.md)保留 160 面固定三维环带，只读核对自由核心的 98 边边界，无交叉；32 点公共环在固定带中每点已有三个邻点，新域必须恰好增加一个。原源接缝 32 对边和 96 对独立双侧查询一致到数值误差范围，但不认证新切缝的连续来源。

本轮只有文档依据与离散接口结果，没有新增模型或放宽四价、75 mm 核心/实际一圈、原双向贴合条件。下一项仅实现并运行一次只读逐三角源域追踪，预算及停止条件见设计 §9.4；前置未完成不进入三角化。App `5dd5b9f`、原 256 点、合同及正式资源保持，零编译/求解/Blender/付费，不新增测试通过数。完整身体、S0、闭合身份、32 骨及动画仍未完成。

### 8.0.59 CHANGE-081 单次源域追踪未完整通过

新切缝完成 1,622 个数值包围区间、65 个原源三角、64 次源边对应；近端原边 0–1 的三角 6274→6273 公共要素检查未能认证，整轮停止，其余 95 段未测。原源边界顶点 3354 位于该固定边的参数内部，但失败版本未保存精确停止参数，不能据此断言全部原因或源曲面有缺陷。

详见[核心约束设计 §10](./pet-character-core-constraint-design.md)。下一项只补原边界顶点事件表示，控制通过后最多一次未完成边界核查；不得调容差重跑本轮旧表示。最终 17 项工具测试通过，真实输入仅运行一次且保持拒绝；无底网/模型/编译/求解/Blender/付费，原合同与正式资源保持，完整身体/S0/身份/32 骨及动画仍未完成。

### 8.0.60 CHANGE-082 边界来源与端点接续完成

剩余 96 段边界的唯一一次来源追踪全部通过，零失败/未测；复用前轮新切缝，对 96 个固定连接点的 194 条端点记录核对通过。最大固定点绑定误差数值上界 0.000015178594 mm，原 0.5 mm 阈值不变；原源顶点事件只作为来源记录，没有增加固定模型顶点。

[核心约束设计 §11](./pet-character-core-constraint-design.md)保留方法、数值限制、31 项工具测试和原 531 项保护文件保全。边界检查完成不等于完整自由域、底网或四边模型认证，S0、身份、32 骨和动画仍未完成。本轮零模型/编译/求解/Blender/付费/依赖；下一项核查原生入口和一份自由域输入，前置通过才最多一次三角化，不重开旧路线或重复边界追踪。

### 8.0.61 CHANGE-083 输入通过，原生入口拒绝

3,736 点核心输入及独立固定边/身份/面积检查通过，原 96 段固定模型边不变，源绑定最大误差 0.000015178570 mm。桥接唯一编译成功，但重复点、固定边拆分等原生错误处理场景 SIGABRT，因此未调用真实三角化，未生成底网或改善模型。

详见[核心约束设计 §12](./pet-character-core-constraint-design.md)。下一项对同份源码验证已有 Apple Clang/libc++，严格错误控制通过才使用本轮冻结输入，不重建点集。最后 Python-only 测试 12 通过、5 原生跳过；573 项保护文件及原生产合同/正式资源保持。S0、身份、32 骨和动画均未放行。

### 8.0.62 CHANGE-084：实际输出审计与 Clang 头文件阻塞

CHANGE-084 完成实际底网输出检查器，最终 28 项 Python 控制通过；唯一 Apple Clang 编译因标准库头文件路径失败，零真实三角化。已只读定位不完整工具链目录遮蔽 SDK 头文件，下一项显式指定已有 SDK 路径并先验证异常处理；模型未改善。

App `5d8592c`；28 项为 Python 工具控制，原生异常/正向控制均未执行，不计产品验收。原 256 固定点、75 mm 核心及真实一圈、160 面环带、合同和正式资源保持。完整源域/S0/身份/骨架门禁未通过。下一项只按[核心约束设计 §13.4](./pet-character-core-constraint-design.md#134-唯一下一项及停止条件)显式选择已有 SDK 标准头，30/60 分钟预算、一次编译及准入后一次真实三角化，失败停止。

### 8.0.63 CHANGE-085：真实底网固定连接通过，三维方向失败

CHANGE-085 已修通原生入口并实际生成 7,244 个三角面；平面覆盖、接缝/固定带连接和 256 点保持通过，但 15 处源法向不一致，真实底网拒绝。已主动复盘并检索原始资料，下一项评估保固定条件的局部连接；未获合格模型。

原 256 点、96 段物理边界、160 面固定带和合同保持。7,244 是实际三角数，不是预测；源法向拒绝项 15 也不是自交/完整折面计数。没有四边模型、完整自交/双向贴合或 S0/身份/骨架通过。App `d79578d` 保存原始失败输出，失败后已做定向互联网研究。后续按[核心设计 §14.4–14.5](./pet-character-core-constraint-design.md)有界评估局部连接，改善不明显时自动复盘检索，不重开旧路线。

### 8.0.64 CHANGE-086：固定邻域换边覆盖失败

CHANGE-086 一次检查 45 条原边，9 个局部替代无法覆盖全部异常，6 个面没有候选，未生成新模型。已自动复盘和检索原始资料；下一项评估仅删除登记的非固定内部采样点并保留其余坐标的有限收缩。模型质量未获改善，原固定点/边/环带保持。

20 项新控制通过，55 面范围的 45 条原边完整枚举；只有 9 个局部替代，6 个目标无候选，因此不组装网格。下一项八点名单、42 面原星形邻域、独立删除账本审计与停止条件见[核心设计 §15](./pet-character-core-constraint-design.md)。App `fdb157d` 保存[21 份证据](../../../engineering/workspace/macos-app/character_pipeline/sprout/v2/work/experiments/sprout-core-local-connectivity-20260922/README.md)。无 S0/身份/32 骨放行，原生产门禁不变。

### 8.0.65 CHANGE-087：局部方向改善与整网证据缺口

CHANGE-087 仅生成一份候选：6 个有限收缩与 9 个换边使同口径每面 7 点方向异常由 30 面/119 点降至 14 面/21 点，重心异常由 15 面降至 0。剩余 14 面都是未改动的原面，没有新增这一类异常；有局部改善，但全网方向仍失败，完整自交/双向贴合未测，整体未通过。下一项先做新旧冻结网格的只读整网几何对照，零新候选。

原 256 个固定点、96 段物理边界、160 个定向固定四边面、源绑定及生产合同保持。删除仅限六个登记的非固定内部采样点，剩余点逐字节保持，并由独立删除账本/原面对应核查。3,730 个计算域点/7,232 三角仍是中间底网，不能作为全四边或核心四价通过。

App `17763d1` 的 30 项新控制通过，[本轮 41 份证据](../../../engineering/workspace/macos-app/character_pipeline/sprout/v2/work/experiments/sprout-core-local-contraction-20260922/README.md)包含真实拒绝网格及同规则新旧对照。剩余方向失败不变更为通过；完整自交、原双向贴合、四边联合约束、身份/UV、32 骨及动作依然分别待验。下一项只读全网诊断的采样规则、限制和停止条件见[核心设计 §16.4](./pet-character-core-constraint-design.md)，不开放蒙皮或动画投入。

### 8.0.66 CHANGE-088：整网反向覆盖失败与路线边界

CHANGE-088 只读整网对照确认：上一轮候选的非相邻面自交由 1 对降至 0，但源→网格最大距离由 4.017495 增至 4.171724 mm，9 个样本超 2 mm，整体仍拒绝。精确下界证明现有两份点集仅靠换连接/删点/凸组合细分不能达标；下一项仅核查六个原源内部参考点的接入与固定边界兼容性，零新候选。

256 个固定点、96 条物理边界、160 个定向固定四边面保持。非相邻面自交由 1 对降至 0，不能覆盖剩余 14 面源法向失败或 9 个反向距离超限；网格仍为中间三角/四边混合体，未获得 S0、四边联合约束、身份/UV 或骨架许可。

App `651be9a` 保存[38 份原始审计与精确证据](../../../engineering/workspace/macos-app/character_pipeline/sprout/v2/work/experiments/sprout-core-global-geometry-20260922/README.md)，24 项新增控制通过。六个源参考点只在报告中登记，尚未加入网格；后续先核查原参数域与固定接口兼容性，不直接制作新候选。精确点集下界及下一项边界见[核心设计 §17.3–17.4](./pet-character-core-constraint-design.md)，不把有限自交检查当作完整生产几何证明。

### 8.0.67 CHANGE-089：六个源重心直接接入拒绝

CHANGE-089 六个原源重心均通过来源、自由域严格内部及精确正面积检查，但直接接入后各产生一个固定边相邻反向面，0/6 准入；停止补点。下一项只读复核三条直边与原源曲线的兼容性及对接影响，形成具体边界表示方案，零新候选。

18 项工具控制通过；真实输入一次，六点均绑定原源并严格位于计算面内部，直接分割精确正面积保持，但每项紧贴固定边的子面七个法向样本全为负。六个独立提案未组装成模型；候选、S0、身份、骨架和动画均不推进。原 14 面方向与九点反向贴合失败保持。

已查四份原始资料，停止调整内点继续试修。下一项为三条源边界曲线与固定段的设计复核：列明可能的接口变更和两侧影响，先完成可审查方案，不直接修改合同。没有整体无解或必须人工接手的结论。详见[核心设计 §18](./pet-character-core-constraint-design.md)及[App 证据](../../../engineering/workspace/macos-app/character_pipeline/sprout/v2/work/experiments/sprout-core-source-admission-20260922/README.md)。

### 8.0.68 CHANGE-090：四点边界提案待确认，生产合同保持

CHANGE-090 只读确认三条源边界与原直段最大差为 2.715 / 2.885 / 4.596 mm；四个已有源事件形成 64→68 点局部切口提案，连续曲线双向误差上界最大 0.175759 mm。方案未启用、零新网格；原 14 面方向及九点贴合问题未修复。下一项须先确认三段局部接口变更，再做一次有界边界验证。

四点来自已追踪的三个真实源角点及一个用于偶数边界的已有事件。原 256 点、160 面和核心定义保持，其他 93 段不动；正式脖子 64 / 腕踝 32 机器合同不变。25 个来源区间有双侧原始材料，但外侧生产补片尚未制作；不能把参考原源当作正式拼接成品。

App `7af2ec4` 新增四个工具/控制文件，28 项合成控制通过，28 份证据归档。下一项仅在明确批准具体局部接口修订后，按 30/60 分钟预算核查三处原面、最多一份隔离三角中间件，失败停止。无正式候选、S0 或骨架准入；详见[核心设计 §19](./pet-character-core-constraint-design.md)及[证据与提案](../../../engineering/workspace/macos-app/character_pipeline/sprout/v2/work/experiments/sprout-core-boundary-representation-20260922/README.md)。

### 8.0.69 CHANGE-091：八类方法验证与配对障碍

一次真实必要条件核查发现 86 个核心点无法仅通过三角配对达到四价；固定导向点 `fixed:832` 的独立枚举给出最终度至少 5。14 个异常面均不能单靠翻转通过。四点边界提案不涉及这些障碍，暂不作为优先施工项；不同布局尚未被排除。

官方获批源 Blend 已恢复并匹配 SHA，冻结 intake 通过，但源接口及关节质量不满足生产直接使用条件，当前许可页面与旧证据差异另行记录。真实源 UV 数据可用；历史 USD 三骨运行时报告与实物 SHA 一致。它们均不代表完整身体、身份、32 骨或最终动作通过。

16 项新控制通过、零新网格。新方案先交付具体全身关节/接口布局，缺前提的下游项明确待验；来源、方法与证据统一见[八类问题的方法与验证清单](./pet-character-method-validation.md)。坐标冲突及闭合身份评估器缺口在该清单中保留，本轮不修改生效合同。

### 8.0.70 CHANGE-092：原件面流已定位，生产布局未完整

只读原件得到 5,626 个原四边面的身体索引视图、185 条规则循环和 175 条验证带；八组六环参考块及实际一圈均四价，最大边长比 2.029002。肩/髋参考块实际位于上臂/大腿，未覆盖腋下/髋裆，也未对应到小草关节核心，不能计作关节通过。

正式关节环不要求统一 32 点；源 22/28 点环可参考，但颈 36→64、腕 22→32、踝 28→32 转换和原固定坐标仍须实现。旧局部 256 点/160 面只属历史试验。胸颈、骨盆、左右肩四个原分支已定位，支撑和目标映射未定义；样件准入失败，新网格 0。14 项工具控制及独立源数组核查不改变 S0、身份、骨架或动作状态。详见[布局、研究和下一项](./pet-character-source-layout.md)；本节不修改任何生产门槛。

### 8.0.71 CHANGE-093：稀疏对应与闭合参考用途

六对原源/小草参考点和四对原边路径已定位；四个目标骨盆/髋裆点未定位，路径存在共享中间点。五接口的 192 个 canonical 点有原源边及参数对应，坐标保持，但零过渡面、零新模型。这些参考路径不满足正式支撑带的声明与不相交条件。

原始小草在精确同坐标分析后满足单组件闭合 genus-0 组合条件，不能视为生产身份或 S0。Surface Maps 作者实现要求闭合输入，先软对齐再冻结特征，并会归一化坐标；尚无正式接口保持或本机执行证据。下一项只补可靠的缺失表面点和相容曲线，见[源布局 §8](./pet-character-source-layout.md)。19 项工具控制及参考数据复算通过，不改变正式门槛和下游禁入状态。
...
当前阻塞点仍是自然变形与表情控制，CHANGE-033 腰部场仍为加工 GLB，历史失败保留。K6 已完成，R1 已拒绝，当前纯权重路线自动投入关闭。CHANGE-064 的 B1 已交付真实静态网格并被独立审计拒绝，1/1 已用；实际范围 5,824 原面、九条边界，结果见 §8.0.43 / 结构方案 §9–10 / 修复计划 §33。四个关节均无合格环，另有 8 个折叠四边面、1 对新增静态自交及双向表面超限；后续权重/动作预算不开放。没有新增可采用的修复，169 条冻结 K3 实际失败保持。下一项为完整定向重建的生产路径决策，原源、正式资源和 S0/32 关节合同保持，没有整体无解或必须人工接手的结论。
