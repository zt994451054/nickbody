...
4. **角色生产**：历史失败路线保持冻结。CHANGE-028/029 已完成首个 H3.1 静态任务和离线评审，消费 35 credits。CHANGE-030 随后按用户新授权完成 H3.1 detailed/P1 两个模型、三次诊断绑定、十段预设动作和一次局部清理对照，净消费 260 credits。H3.1 外形较好；两份通用 rig 动作失败，人形 v1.0 控制有改善但肩腋/躯干问题未解决，Mixamo retarget 被服务拒绝，表情未提供。身体筛选未通过，条件性贴图/交付减面/RealityKit 接入未启动。下一轮建议验证 H3.1 有限减面、人形原生命名绑定与预设动作，同时处理肩髋变形。生产 formal candidate 仍须通过 open-five S0、闭合身份与用户静态审核后才能进入固定骨架、蒙皮和 Gate 0–3；诊断产物不直接替换正式资源。实测与实图入口见 Rig v2 §8.0.10。
