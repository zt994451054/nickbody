...
模拟 float32 权重和坐标仍会漏掉实际保护失败，Blender 验收继续必需。下一项核验新增接触
有界可达性，再接入完整清单、新谓词及步骤修正，见修复计划 §21 / Rig v2 §8.0.31。
K1 未登记，无新增真实模型 QP，不自动 C4；没有模型整体无解证明。
...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-052 修复源接触漏检并补全 1,534 条恢复清单，49 项测试及新源 R/F 准备通过，同方向 CCD 安全比例约提升 6.45 倍；C3 实际造型仍拒绝。下一项核验新增接触可达性并接入新谓词/步骤修正与 Blender 验收，见修复计划 §21；未证明模型无解，不自动 C4。P2 2/2、S 2/2、C1/Q1/C2/C3 各 1/1，K1 未登记，D 未启动。表情、条件性贴图与 RealityKit 未完成。正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3，见 Rig v2 §8.0.31。
