...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-059 确认 K3 锚点预测误报局部已知源的 6 条折角；73 项测试通过，无新增候选/QP/Blender。骨矩阵一致，坐标偏差和采集往返链路待实测隔离，非线性仍不能排除。下一项 K6 双路径配对采样，见修复计划 §28 / Rig v2 §8.0.38；采样坐标函数已测试，驱动/登记待完成。模型未修好，不把局部源反例当作完整修复可行。旧预算保持，K6 未登记，不自动 C4，D 未启动。表情、条件性贴图与 RealityKit 未完成；正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3。
