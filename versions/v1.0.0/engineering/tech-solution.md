...
下一项方案应将极端折回和碰撞纳入显式约束并定义收敛退出标准，预算需另行确定，
不自动追加 S3，也无证据要求人工接手或重拓扑。详见修复计划 §13 / Rig v2 §8.0.23。
...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-044 已完成共同方向检查和一次 S2；尖折边减少但新增相交、深折仍在，左膝未修好。P2 2/2、S 2/2，D 未启动。下一项为显式折回/碰撞约束及收敛退出的方案，不自动追加预算或判定必须人工/重拓扑，见修复计划 §13。表情、条件性贴图与 RealityKit 未完成。正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3，诊断产物不替换正式资源，见 Rig v2 §8.0.23。
