...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-053 接触前置检查、完整 R/F 接入及 95 项新旧测试通过。K1 数学路径通过，实际分离 4 对源接触；但 Blender 113 条保护折角失败，候选拒绝。下一项验证实际求值差异，见修复计划 §22；未证明模型无解，不自动 C4。P2 2/2、S 2/2、C1/Q1/C2/C3/K1 各 1/1，D 未启动。表情、条件性贴图与 RealityKit 未完成。正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3，见 Rig v2 §8.0.32。
