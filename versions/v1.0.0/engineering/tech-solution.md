...
下一项补足失败 x/y 与逐行残差记录、等价缩放合成验证，再独立登记首个 QP 回放，
见修复计划 §16 / Rig v2 §8.0.26。P2 2/2、S 2/2、C1 1/1，不自动重启。
...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。左膝未修好；CHANGE-047 求解器及 41 项测试完成，真实 C1 在第一个 QP 内迭代上限退出，接受步 0。P2 2/2、S 2/2、C1 1/1，D 未启动。下一项为失败 QP 诊断、等价缩放合成验证及独立回放登记，见修复计划 §16；本轮没有替换权重或新模型。表情、条件性贴图与 RealityKit 未完成。正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3，诊断产物不替换正式资源，见 Rig v2 §8.0.26。
