...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-062 的 R1 未通过，当前纯权重路线自动投入关闭；R2/R3 不启动、试修使用 0/1。源点固定与目标穿插冲突，源基准预测仍误放行六处已有实际失败；下一项转骨位/rest 与局部定向布线的结构方案比较。详见 R1 结论 / 路线评审 §9 / 修复计划 §31 / Rig v2 §8.0.41。模型仍有 169 条实际保护失败，原合同及正式资源保持。表情、条件性贴图与 RealityKit 未完成；结构方案须明确生产适配代价，正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3。
