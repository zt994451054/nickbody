...
4. **角色生产**：当前接续为 CHANGE-069 [固定图布点结果 §8](./pet-character-shoulder-migration-study.md)。精确缓冲已落实，但实际网格自交 143 对、核心一圈 10 个极点，贴合退步；91 个不同折面仍失败，没有整体模型进展。下一项先明确源曲面保护/转换分区及不交叉边界，再联合设计连接与布点。全身工单保持，完整身体未开工。正式身份不变，Tripo 41 骨仅作试验/运动参考，CHANGE-033 为 vendor 加工 GLB；B1、权重及旧路线关闭，旧余额不重开。详见 Rig v2 §8.0.48。S0、闭合身份/UV、用户审核、32 关节、Gate 0–4 与运行时往返仍未完成；冻结 K3 的 169 条失败保持。最后核实余额 190，本轮未查询或消费，正式合同/资源保持。
