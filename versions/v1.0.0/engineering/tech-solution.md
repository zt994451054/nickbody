...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-060 K6 一次 Blender 精确复现源/K3；直接采集仍有相同 169 条失败和 6 条已知源预测误报。59 项测试通过，无新权重/QP。采集器替换方案已排除，下一项只读审计失败保护点保留源权重的条件，见修复计划 §29 / Rig v2 §8.0.39。模型未修好，无整体无解证明。K6 已用 1/1，K7 未登记，不自动 C4，D 未启动。表情、条件性贴图与 RealityKit 未完成；正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3。
