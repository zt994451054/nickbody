...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-061 确认最近模型停滞、诊断推进；停止滚动追加 K 诊断，按路线评审以 300 分钟后续总预算和一次新试修控制投入。先形成完整方案，失败转结构方案选择；源权重保留核查已有 7 点在当前域外，不能直接试修。详见路线评审 / 修复计划 §30 / Rig v2 §8.0.40。模型未修好，K7 未登记，D 未启动。表情、条件性贴图与 RealityKit 未完成；41 骨试验成果的生产适配须先说明，正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3。
