...
第一固定 QP 可解，尚未验证完整非线性修复。下一项将通过的边界接入新 R/F 路径、回归并冻结完整试修预算，
见修复计划 §17 / Rig v2 §8.0.27。P2 2/2、S 2/2、C1 1/1、Q1 1/1；QP 方向未应用模型。
...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-048 完成失败诊断及 27 项测试，Clarabel 30 内步通过首个 QP，原单位残差合格；完整左膝修复尚未验证。P2 2/2、S 2/2、C1 1/1、Q1 1/1，D 未启动。下一项接入新 R/F 边界、回归并冻结完整试修预算，见修复计划 §17；无新蒙皮或模型替换。表情、条件性贴图与 RealityKit 未完成。正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3，诊断产物不替换正式资源，见 Rig v2 §8.0.27。
