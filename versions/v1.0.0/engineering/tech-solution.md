...
下一项先实现合成/只读测试与冻结折边的原图定位；固定目标冲突须先修订范围。
一次新增试修预算提案及收敛/停滞退出见修复计划 §14 / Rig v2 §8.0.24，额度尚未确定。
离线 OSQP/IPC Toolkit 为拟议工具依赖，尚未安装；运行时架构、基础设施和生产合同不变。
...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-044 的 S2 因新相交/深折拒绝，左膝未修好；P2 2/2、S 2/2，D 未启动。CHANGE-045 已完成约束实施单和只读清单；下一项是工具合成/只读验证及冻结折边原图定位，新试修额度尚未确定，见修复计划 §14。表情、条件性贴图与 RealityKit 未完成。正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3，诊断产物不替换正式资源，见 Rig v2 §8.0.24。
