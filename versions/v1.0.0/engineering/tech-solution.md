...
4. **角色生产**：CHANGE-024 source intake 与 CHANGE-025 v001 失败证据均已冻结；CHANGE-026 licensed-basemesh AI 施工路线已关闭，CHANGE-027 的 Tripo P1 四视图因旧 24 骨权重污染在付费前被拒绝，Tripo 未提交、未评估且消费 `0 credits`。CHANGE-028 阶段 0 已从冻结身份 GLB 的 `char1` 原始 mesh data 创建未经过 armature 求值的静态副本，保持 `Head=1.0` 和 rest 双上臂约 `52.6°/55.6°`，完成四视图机器门与逐张原尺寸审核并交用户复核；本地流程未上传或创建 task，账户侧用量未查询。阶段 1 的 H3.1 无纹理多视图候选固定为 `v3.1-20260211`、Quad + Smart Low-poly、10,000 面和 `model_seed=424242`，未核验计划估值为 `35 credits`；只有输入获用户批准、调用前价格可验证且单次不超过 `50 credits` 后，才可请求新的执行授权。不得使用实际请求试价、`tripo ai`、`make`、MCP `tripo_make`、自动重试或多候选；获批后唯一允许的提交路径是显式锁定模型和全部参数的 CLI `generate multiview-to-model`。后续纹理、Rig Check、Auto Rig、权重和动作均须独立授权。任何 Tripo 输出仍为 `scoped_wip`，不能创建 v002 或直接进入 formal S0。未来 formal candidate 仍须依次通过 open-five S0、闭合身份装配与用户静态审核，之后才能建立 control/export 双骨架并手工蒙皮，再按 Gate 0–3 放行。
