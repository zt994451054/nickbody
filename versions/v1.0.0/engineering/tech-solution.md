...
后续优先固定该几何和动作检查局部权重/骨位，详见 Rig v2 §8.0.11。
...
4. **角色生产**：历史失败路线保持冻结。CHANGE-028/029 首个静态模型消费 35 credits，CHANGE-030 生成/绑定整轮消费 260。CHANGE-031 完成两种减面对照，选中 basic 80k，获得人形 v1.0 / Tripo native 41 骨和五段预设动作，消费 115，余额 190。供应商处理链路已跑通，但肩髋/胸腹和远端背部拉扯仍不合格，面部控制缺失；未执行条件性贴图或 RealityKit。后续固定几何/动作检查骨位与权重影响，按证据确定局部修整范围。生产 formal candidate 仍须通过 open-five S0、闭合身份与用户静态审核后进入固定骨架、蒙皮和 Gate 0–3；诊断产物不直接替换正式资源。实测与实图见 Rig v2 §8.0.11。
