...
4. **角色生产**：CHANGE-024 source intake 与 CHANGE-025 v001 失败证据均已冻结；CHANGE-026 licensed-basemesh AI 施工路线已关闭，CHANGE-027 的 Tripo P1 四视图因旧 24 骨权重污染在付费前被拒绝，Tripo 未提交、未评估且消费 `0 credits`。CHANGE-028 已从冻结身份 GLB 的 `char1` 原始 mesh data 创建 `Head=1.0`、rest 双上臂约 `52.6°/55.6°` 且未经 armature 求值的四视图，机器、原尺寸和用户审核均通过。阶段 1 精确固定为单次 `v3.1-20260211` H3.1 无纹理 multiview-to-model、Quad + Smart Low-poly、10,000 面和 `model_seed=424242`；官方公开价目已绑定为 `35 credits`，`50 credits` 仅为审批阈值而非服务端上限，提交前仍须重新验证。默认 CLI/MCP、自动重试与多候选均禁用；唯一允许路径是 App `7c87e8c` 的项目守卫，以 `claim → balance → 四图上传 → durable create_intent → single createTask` 顺序、`maxRetries=0` 执行，并对凭证读取、provisional CDN、URL hash-only 停止和 `512 MiB / 1 GiB` 产物边界 fail closed。守卫已通过独立安全复审、Stage 1 `111/111` 和 Stage 0 `27/27`；但阶段 1 仍须独立的一次性执行授权，并由用户选择轮换凭证或接受第三方已披露凭证的非独占风险。到当前为止未读取凭证、查询账户、上传或创建 task。后续纹理、Rig Check、Auto Rig、权重和动作均须独立授权；任何 Tripo 输出仍为 `scoped_wip`，不能创建 v002 或直接进入 formal S0。未来 formal candidate 仍须依次通过 open-five S0、闭合身份装配与用户静态审核，之后才能建立 control/export 双骨架并手工蒙皮，再按 Gate 0–3 放行。
