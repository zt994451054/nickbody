...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB。CHANGE-049 完整 C2 接受 23 步后在 R 停滞，QP 数值全通过但原图造型拒绝；6 处固定子特征穿越证明当前冻结恢复范围不可行。P2 2/2、S 2/2、C1/Q1/C2 各 1/1，D 未启动。下一项重建膝—髋接触区域与保护边界，见修复计划 §18；隔离候选不替换源。表情、条件性贴图与 RealityKit 未完成。正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3，见 Rig v2 §8.0.28。
