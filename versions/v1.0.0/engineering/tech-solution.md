...
CHANGE-043 仅重排计划，尚未实施：先核对源数据和四姿势共同合法方向，固定一个联合权重
方案后才使用 S2；形状由同一份合法权重产生，不继续独立雕刻 D1/D2 式静态目标。
人工接手不再作为前提。首次求新权重或内存预览就计 S2，P2 2/2、S 1/2 不重置；
一次求解通过四姿势后才做连续性及既定交付路径验证。失败按确切约束/区域列下一方案，
不自动扩大范围或推断必须重拓扑。执行细节见修复计划 §12 / Rig v2 §8.0.22。
本轮无新代码、计算/造型试验、权重、资源、架构或依赖变更。
...
4. **角色生产**：历史失败路线保持冻结，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通，CHANGE-033 腰部场保留加工 GLB；后续 P2/S1/D1/D2 均未修好左膝。CHANGE-043 只重排计划：AI 先检查共同合法方向，固定一个联合权重方案后条件性使用 S2，人工接手不是必要前提；计划 §12 尚未实施。P2 2/2、S 1/2，S2 未启动；左侧通过后才扩展连续动作、交付和右侧。表情、条件性贴图与 RealityKit 未完成。正式候选仍须通过 open-five S0、闭合身份、用户静态审核及固定骨架/Gate 0–3，诊断产物不替换正式资源，见 Rig v2 §8.0.22。
