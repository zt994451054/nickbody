...
4. **角色生产**：历史失败路线保持冻结。CHANGE-028/029 消费 35 credits、CHANGE-030 消费 260、CHANGE-031 消费 115，最后核实余额 190。basic 80k / native 41 骨及五动作链路已跑通。CHANGE-032～041 本地工作未新增消费；腰部场保留加工 GLB。CHANGE-038 下缘仅留参考，CHANGE-039 蒙皮不采纳，CHANGE-040 排除单改接收比例。CHANGE-041 D1 目标因残留台阶、跳跃折痕和新增左手接触不采纳，现有骨影响不能精确表达该组目标；左膝未通过。P2 2/2、S 1/2，S2 未启动；定点造型及手部避让见修复计划 §10。左侧稳定且通过连续性/交付可行性后再看右侧与原动作。表情控制缺失，条件性贴图和 RealityKit 未执行。生产 formal candidate 仍须通过 open-five S0、闭合身份与用户静态审核后进入固定骨架、蒙皮和 Gate 0–3；诊断产物不替换正式资源。实测与实图见 Rig v2 §8.0.20。
